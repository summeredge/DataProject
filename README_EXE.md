...
该脚本先验证 EXE 的 XGBoost、SHAP、`openpyxl` 和 `xlrd` 导入；随后无参数启动桌面模式，检查桌面主进程及其本地服务子进程。自动测试在关闭主窗口前会分别等待本地首页 HTTP 200 和桌面主窗口创建完成，因此 HTTP 服务就绪与桌面窗口就绪会被独立验证；随后通过关闭主窗口验证应用自身清理，确认主进程、子服务和动态端口均已退出或释放。`taskkill` 仅用于测试失败后的兜底清理：子服务兜底会区分已正常退出与 PID 身份变化，避免误导日志和误杀其他进程。最后用内部服务模式执行上传、列识别、最小分析、结果查询和下载，并在关闭后检查 8765 端口已释放。正常用户启动不需要任何参数。最终仍需在真实 Windows 电脑上人工点击关闭按钮，确认没有残留进程。若桌面主进程异常退出但本地服务残留，冒烟测试会在核对进程身份后单独清理该服务。
