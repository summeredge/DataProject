...
def build_markdown_summary(
    ...
    lines.extend(
        [
            ...
            "- 增强筛选、Granger、模型分析和可信度审查须在对应页面单独运行后解读；可信度审查用于识别独立预测贡献、混杂风险、控制关系和统计限制。",
            "- 第三层运行后可查看 evidence_matrix.csv：它按变量、初筛结果、预测价值证据、独立性、混杂风险、控制关系和统计限制组织人工复核信息，不是排名或综合评分。",
            "- final_review_summary.csv 中的 final_rank 仅表示人工复核优先级（展示序号）；final_recommendation 等字段表示复核建议，证据字段仅作证据摘要，不参与初筛评分或排序。",
