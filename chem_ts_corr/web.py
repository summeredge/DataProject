...
const termsHelpRows = [
  ...
  { category: "风险标签说明", name: "数据质量风险", signal: "数据质量需关注：存在普通质量问题，得分已通过数据质量系数平滑降低。数据质量严重不足：存在极端质量问题，除连续降分外，最终得分上限为 0.44。", reading: "统计指标可能受采样、坏点或仪表状态驱动。", action: "先清洗数据、确认仪表有效性，再重新运行分析。" },
