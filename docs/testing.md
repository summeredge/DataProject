...
- 启动分析必须快照当前窗口；空窗口与未使用窗口的基线输入等价，窗口只在下一次分析生效；
- 初筛的 Raw 与 Processed 分支必须使用同一批排除后的时间点；窗口边界的行被排除，原始
  上传文件不变；
- 排除形成的物理断点不得被低通、插值、前向填充、去趋势、差分或滞后跨越；
- `preprocessing_context.json` 必须冻结窗口与行数统计；恢复/修改当前 Web 窗口后，
  downstream 仍使用该运行的冻结窗口，且不得改变 `final_score`、排序、Top-K 或候选池。
