...
默认最多选择 8 个自动候选，白名单候选可在自动候选之外强制加入。设有效候选数为 `C`、时间折数为 `F`，完整流程训练次数为：

```text
(3 + C) x F
```

前三项对应 M0、M1、M2；候选模型为 `C x F`。候选数量与运行时间近似线性增长。XGB 训练可能占用较多 CPU，建议先使用默认 TopN 8 和默认模型参数。`max_lag` 必须在 1～5000 之间；过大会减少有效完整样本并扩大时间 gap，不应盲目增加。
