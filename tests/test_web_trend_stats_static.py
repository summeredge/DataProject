...
def test_trend_chart_width_uses_container_measurement():
    assert 'function trendChartWidth(container)' in INDEX_HTML
    assert 'container.getBoundingClientRect().width' in INDEX_HTML
    assert 'parentElement.getBoundingClientRect().width' in INDEX_HTML
    ...
    assert 'renderTrendChart(lastTrendSeries, lastTrendAxisMode)' in INDEX_HTML
